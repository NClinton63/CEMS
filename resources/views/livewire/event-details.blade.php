    ...
    @if($event)
        @if($event->featured_image)
            <div class="relative h-96 bg-gray-900">
                <img src="{{ asset('storage/' . $event->featured_image) }}" alt="{{ $event->title }}" class="w-full h-full object-cover opacity-90">
                    ...
                    <div class="card p-8">
                        ...
                        <div class="flex items-center">
                            <div class="w-12 h-12 bg-gradient-to-br from-primary-400 to-primary-600 rounded-full flex items-center justify-center mr-4">
                                <span class="text-white font-bold text-lg">{{ substr($event->organizer->name, 0, 1) }}</span>
                            </div>
                            <div>
                        ...
                        <div class="space-y-4 mb-6">
                            <div class="flex items-start">
                                ...
                                <div>
                                    ...
                                    <p class="font-semibold text-gray-900">{{ $event->start_time->format('l, F d, Y') }}</p>
                                </div>
                            </div>
                            ...
                            <div class="flex items-start">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500 mt-1">{{ ucfirst($event->location_type) }}</p>
                                </div>
                            </div>
                            ...
                            <div class="flex items-start">
                                ...
                                <div>
                                    ...
                                    <p class="font-semibold text-gray-900">{{ $event->availableSpots() }} / {{ $event->capacity }} spots</p>
                                    <div class="w-full bg-gray-200 rounded-full h-2 mt-2">
                                        <div class="bg-primary-600 h-2 rounded-full" style="width: {{ ($event->capacity - $event->availableSpots()) / $event->capacity * 100 }}%"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div class="bg-green-50 border-l-4 border-success rounded-lg p-4 mb-4">
                                        <div class="flex items-start">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-sm text-green-700 mt-1">Registered on {{ $userRegistration->created_at->format('M d, Y') }}</p>
                                            </div>
                                        </div>
                                    </div>
